...
app.get('/home', async (req, res) =>  {
	if (req.session.loggedin) {
	    ...
	    SELECT *
		FROM stats S INNER  JOIN (SELECT username, exp, type, shape, color
			FROM users WHERE username = '${req.session.username}') B
			ON S.username = B.username;`
		);

		const top5 = await client.query(`
		SELECT username, kills, deaths from stats ORDER BY kills DESC limit(5);
		`);
...
app.post('/editUser', (req,res) => {
    ...
    if(kills && deaths && exp) {
        ...
        var results = {'result':'Please enter stats to edit!'};
        res.render('pages/admin',results);
    }
});
